...
class BookingController extends Controller
{
    ...
    public function completeBooking($id)
    {
        $book1 = new Booking;
        $book = DB::table('bookings')->where('id', $id)->get();
        return view('Admin.CompleteBookingView', ['book' => $book, 'book1' => $book1]);
    }
    public function completeBookingPost(Request $request)
    {
        try {
            $id = $request->input('IdBookComplete');
            $book = Booking::find($id);
            $book->cost = $request->input('costBook');
            $book->status = 2;
            $book->save();
            return redirect(route('admin.book'))->with('notice', 'Cập nhật lịch hẹn thành công');
        } catch (Throwable $e) {
            return redirect(route('admin.book'))->with('error', 'Có lỗi xảy ra !');
        }
    }
}
